...
### Step 0: 前置豁免 — 纯 prompt / 规则 / 文档仓库

某些仓库**根本没有运行时代码**，只承载 prompt / 配置 / 文档（如 Claude Code 插件、Cursor rules、agent 提示词集合、纯 markdown 知识库）。这类仓库**不该被搭测试基建**——没有运行时栈，hello world 测什么？依赖装到哪个 `package.json`？

**判定为豁免对象**（满足以下**任一**且根目录**没有任何运行时项目描述文件** —— `package.json` / `pyproject.toml` / `go.mod` / `Cargo.toml` / `foundry.toml` / `pubspec.yaml` / `Gemfile` / `composer.json` / `pom.xml` / `build.gradle` 都不存在）：
...
| 根目录 `mcp.json` / `mcp_config.json` 但无任何业务代码 | 纯 MCP 配置仓库 |
| `**/*.md` 占源文件 >80% 且无运行时项目描述 | 纯 markdown 知识库 |

> ⚠️ **同时拥有运行时代码** 的混合仓库（如插件 manifest + TS 源码 + `package.json`）**不豁免**，正常走 Step 1。
...
ℹ️ /tc-test-setup 不适用 —— 没有运行时栈可搭测试基建。

🤔 这类仓库的质量保障应是验证**内容**而不是跑代码测试，推荐工具（**本命令不会自动装**）：
   ✓ markdown-link-check / lychee     验证内部 / 外部链接能跳到
   ✓ markdownlint                     markdown 格式规范
   ✓ prettier --check "**/*.md"       格式一致性
   ✓ 自定义脚本                       校验 frontmatter 字段完整性、命令与 Skill 互引用是否 resolve

📌 真正的"端到端验证"是**人工演练**：在 Claude Code 里实际跑一遍命令/skill，
    看 Claude 是否按 prompt 的预期行为响应。这不是 /tc-test-setup 的职责范围。

⛔ /tc-test-setup 在此结束，未做任何修改。
```

满足豁免 → **STOP**，不进入 Step 1。不满足 → 继续 Step 1。
