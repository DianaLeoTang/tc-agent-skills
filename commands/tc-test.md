...
### Step 1.5: 确定本次跑测对应的 **feature 目录**（强制，决定报告落盘位置）

**为什么必须先确定 feature**：测试报告 + 覆盖率报告**收敛到对应需求文件夹下**（`{SPECS_DIR}/N.{feature}/`），不再散落到项目根。一个项目下可能有多个 feature 在并行/串行迭代，把 `.tc-test-report.md` 丢在项目根会被后跑的 feature 覆盖，并且看不到"是哪次需求的产物"。
...
#### 1.5.2 选定 `FEATURE_DIR`（按优先级，第一个命中即用）

| 优先级 | 来源 | 命中条件 |
|---|---|---|
| 1 | 用户显式 | `/tc-test feature N` 中的 `N` → 取 `{SPECS_DIR}/N.*/` |
| 2 | 当前 git 分支 | 分支名形如 `feature-N-{name}` / `feat/N-{name}` → 抽 N → 取 `{SPECS_DIR}/N.*/` |
| 3 | 最近修改 | `{SPECS_DIR}/` 下编号目录中 `tasks.md` mtime 最新的那个 |
| 4 | 编号最大 | 上面都判不出 → 取编号最大的 `{SPECS_DIR}/N.*/` |

输出强制：

```text
🎯 报告归属 feature: 2.user-login (匹配自 git 分支 feature-2-user-login)
   报告路径: docs/tc-spec/2.user-login/test-report.md
   覆盖率路径: docs/tc-spec/2.user-login/coverage-report.md
```

如果命中的来源不是用户显式（来源 2/3/4），且 `SPECS_DIR/` 下有多个候选 feature → **必须先和用户确认归属**，否则可能写错文件夹：

```text
⚠ 检测到多个 feature 候选，无法确定本次报告归属:
   1) 1.jiyuan-channel
   2) 2.user-login (推荐 — tasks.md 最新)
   3) 3.payment

请回复编号确认（或重新跑 `/tc-test feature N`）
```

#### 1.5.3 确认 `FEATURE_DIR` 存在且是有效 specs

`FEATURE_DIR` 下必须有 `tasks.md` 或 `requirements.md` 之一才认。否则视作"无对应 specs"，走 1.5.4 兜底。

#### 1.5.4 兜底：项目根本没建 specs

如果 `SPECS_DIR` 不存在，或下面没有任何 `N.{name}/` 目录 → **不要回退到把报告写到项目根**（这是用户明确反对的行为）。改为：
...
- ❌ **绝不允许在 feature 归属不明时盲选** —— 多个 feature 候选时必须先和用户确认（参考 1.5.2），写错文件夹会污染其它需求
