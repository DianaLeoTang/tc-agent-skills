...
### Step 4: 总结报告（双输出：终端 + markdown 文件）

全量跑完（或 fail-fast 停止）后**同时做两件事**：

1. **打到终端**（用户即时看）
2. **写一份 `.tc-test-report.md` 到项目根**（用户事后回看 / 贴 PR / CI artifact）

> 💡 **为什么默认写文件而不是开关**：参数多用户记不住，记不住就不会用，加了等于没加。报告默认产出，用户看不需要时再删，不需要记任何参数。
...
📄 报告已生成: .tc-test-report.md
   ├─ 含: 每层结果 / 失败用例 / 覆盖率数字 / 各种 HTML 报告路径
   ├─ 用途: 贴到 PR 描述 / 上传 CI artifact / 历史对比（每次覆盖同一文件）
   └─ 提示: 如未加 .gitignore 建议加上（`.tc-test-report.md`）
...
#### 4.2 `.tc-test-report.md` 文件结构

每次跑完**覆盖写**到项目根目录的 `.tc-test-report.md`（固定文件名，不带时间戳，避免堆积）。文件内容比终端更全：
...
---
*由 `/tc-test` 自动生成 · 下次跑测会覆盖此文件*
```

#### 4.3 关于 `.tc-test-report.md` 与 git

**强约束**：

- ✅ 默认写到项目根 `.tc-test-report.md`（点前缀，ls 默认不显示，污染感低）
- ✅ 终端输出**必须**包含"建议加 .gitignore"提示（如果还未在 .gitignore 中）
- ❌ **不**自动改 .gitignore（避免无声修改用户文件）—— 只提示
- ❌ **不**主动 commit / push 报告文件
- ⚠️ 报告每次跑都覆盖（不存历史，避免堆积）；想存档由用户自己 mv

**对应的 .gitignore 建议片段**（终端提示用户复制）：
...
| **L1：完全没有测试框架** | 输出情形 C 引导（推荐 `/tc-test-setup` 一键搭建）|
| **L2：测试脚本未配** | 输出情形 C，给"在 package.json / pytest.ini 加 test 入口"的最小修复方案 |
